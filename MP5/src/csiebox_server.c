...
char fifo_path[128];
int fifo_fd;
void handle_web_req(int sig){
	uint32_t buf = htonl(active_thread_num);
	
	fifo_fd = open(fifo_path,O_WRONLY);
	write(fifo_fd,&buf,sizeof(uint32_t));
	close(fifo_fd);
}
void quit(int sig){
	//close(fifo_fd);
...
void csiebox_server_init(csiebox_server** server, int argc, char** argv) {
  ...
  *server = tmp;
	...
	if(argc==3 && strcmp(argv[2],"-d")==0){
		...
		if(pid>0){
			char server_pid_path[128]={};
			sprintf(server_pid_path,"%s/csiebox_server.pid",tmp->arg.run_path);
			int fd = open(server_pid_path,O_WRONLY|O_TRUNC|O_CREAT,0666);
			char pid_s[64] ={};
			sprintf(pid_s,"%d\n",pid);
		...
		if(setsid()<0){
			fprintf(stderr,"setsid err!\n");
			exit(1);
		}
	}
	memset(fifo_path,0,sizeof(fifo_path));
	sprintf(fifo_path,"%s/fifo.%d",tmp->arg.run_path,getpid());
